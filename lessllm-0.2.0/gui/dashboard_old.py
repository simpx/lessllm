...
def main():
    ...
    if not recent_df.empty and event.selection and event.selection.rows:
        selected_row_idx = event.selection.rows[0]
        selected_request_id = recent_df.iloc[selected_row_idx]['request_id']
    elif not recent_df.empty:
        # 如果没有选择，默认选择第一行
        selected_request_id = recent_df.iloc[0]['request_id']
    
    # 显示选中请求的详情
    if selected_request_id:
        st.markdown("### 🔍 请求详情")
        st.info(f"当前查看：{selected_request_id} (点击表格行可切换)")
